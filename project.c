...
int Diagnostic( FILE* fileVelocity, FILE* fileT, FILE* fileVorticity, matrix* velocity, matrix* T, matrix* vertocity, int m, int n)
{
    double velocityAverage = 0, TAverage = 0, vertocityAverage = 0;
    int i; 
    for ( i = 0; i<m*n; i++){
        velocityAverage += velocity->data[i];
        TAverage += T->data[i];
        vertocityAverage += vertocity->data[i];
    }
    velocityAverage /= (m*n);
    for (i = m*n; i <(m+1)*(n+1); i++)
    {
        TAverage += T->data[i];
        vertocityAverage += vertocity->data[i];
    }
    vertocityAverage /= ((m+1)*(n+1));
    for (i = (m+1)*(n+1); i < (m+2)*(n+2); i++)
    {
        TAverage += T->data[i];
    }
    TAverage /= ((m+2)*(n+2));
    fprintf(fileVelocity,"%le;\n",velocityAverage);
    fprintf(fileT,"%le;\n",TAverage);
    fprintf(fileVorticity,"%le;\n",vertocityAverage);
    ...
    for (i = 1; i < n; i++){
        for (j = 1; j< m+1; j++ ){ // decalle les indices des V de 1 vers la droite 
            H_x->a[i][j] = (1.0/(4.0*deltax) * (U->a[i+1][j]*U->a[i+1][j] - U->a[i-1][j]*U->a[i-1][j]) 
                          + 1.0/(4.0*deltay) * ((V->a[i][j] + V->a[i+1][j]) * (U->a[i][j+1] - U->a[i][j]) + (V->a[i][j-1] + V->a[i+1][j-1]) * (U->a[i][j] - U->a[i][j-1]))
                          + 1.0/(4.0*deltax) * ((U->a[i+1][j] + U->a[i][j]) * (U->a[i+1][j] + U->a[i][j]) - (U->a[i][j] + U->a[i-1][j]) * (U->a[i][j] + U->a[i-1][j]))
                          + 1.0/(4.0*deltay) * ((U->a[i][j+1] + U->a[i][j]) * (V->a[i+1][j] + V->a[i][j]) - (U->a[i][j] + U->a[i][j-1]) * (V->a[i+1][j-1] + V->a[i][j-1])))/2.0;
        }
    }
    for (i = 1; i < n+1; i++){
        for (j = 1; j< m; j++){ // decalle les indice des U de 1 vers le le haute
            H_y->a[i][j] = (1.0/(4.0*deltay) * ((U->a[i-1][j+1] + U->a[i-1][j]) * (V->a[i][j] - V->a[i-1][j]) + (U->a[i][j+1] + U->a[i][j]) * (V->a[i+1][j] - V->a[i][j]))
                         + 1.0/(4.0*deltax) * (V->a[i][j+1]*V->a[i][j+1] - V->a[i][j-1]*V->a[i][j-1])
                         + 1.0/(4.0*deltax) * (((U->a[i][j+1] + U->a[i][j]) * (V->a[i+1][j] + V->a[i][j])) - ((U->a[i-1][j+1] + U->a[i-1][j])*(V->a[i][j] + V->a[i-1][j])))
                         + 1.0/(4.0*deltay) * ((V->a[i][j+1] + V->a[i][j]) * (V->a[i][j+1] + V->a[i][j]) - (V->a[i][j] + V->a[i][j-1]) * (V->a[i][j] + V->a[i][j-1])))/2.0;
        }
    }
    return 0;
}
...
int evalEstimVelocity(matrix *U, matrix *V, matrix *Hy, matrix *Hx, matrix *LapU, matrix *LapV,
    ...
    for (i = 1; i < n+1; i++){
        ...
        Uestim->a[i][m+1] = -0.2 *(Uestim->a[i][m+1-3] - 5.0*Uestim->a[i][m+1-2] + 15.0*Uestim->a[i][m+1-1] -16.0*Uestim->a[i][m]);
    }
...
int evalVelocity(matrix *phi, double dt, double deltax, double deltay, matrix *U, matrix *V, int m, int n, matrix *Uestim, matrix *Vestim){
    ...
    for (i = 0; i<n+1; i++){
        U->a[i][0] = -0.2 *(U->a[i][3] - 5.0*U->a[i][2] + 15.0*U->a[i][1]);
        U->a[i][m+1] = -0.2 *(U->a[i][m+1-3] - 5.0*U->a[i][m+1-2] + 15.0*U->a[i][m] - 16.0*U->a[i][m]);
    }
    for (j = 0; j< m+1; j++ ){
        V->a[0][j] = -0.2 *(V->a[3][j] - 5.0*V->a[2][j] + 15.0*V->a[1][j]);
        V->a[n+1][j] = -0.2 *(V->a[n+1-3][j] - 5.0*V->a[n+1-2][j] + 15.0*V->a[n+1-1][j]);
    }
    return 0;
}  //checked 
...
int main(int argc, char *argv[]){
    ...
    if(Sim==0){
        ...
        FILE* fileVelocity = fopen("results/VelocityAverage.txt","w");
        if (fileVelocity == NULL) {
            printf("Error : cannot create result file : did you create the output directory ? \n");
            exit(0); }
        FILE* fileVorticity = fopen("results/VorticityAverage.txt","w");
        if (fileVorticity == NULL) {
            printf("Error : cannot create result file : did you create the output directory ? \n");
            exit(0); }
        FILE* fileT = fopen("results/TAverage.txt","w");
        if (fileT == NULL) {
            printf("Error : cannot create result file : did you create the output directory ? \n");
            exit(0); }
        Diagnostic(fileVelocity, fileT, fileVorticity, NormVelocity, T, Vorticity, m, n);
        double l0 = 1e-3*H ;
        for (int i=1;i<n+1; i++){
        //Free Surface : 
        T->a[i][m+1] = ((2*l0-deltay)/(2*l0+deltay))*T->a[i][m];
